<script>
  ...
  let canBuild = $derived(files.length >= 1 && state !== 'working');
  ...
  function startBuild() {
    ...
    const worker = new Worker(new URL('./imagesToPdf.worker.ts', import.meta.url), {
      type: 'module',
    });
...
<Dropzone accept="image/png,image/jpeg,.png,.jpg,.jpeg" on:files={handleFiles} />
...
{#if files.length > 0}
  ...
  {#if files.length > 1}
    <p class="hint">{copy.reorderHint}</p>
  {/if}

  <button onclick={startBuild} disabled={!canBuild}>{copy.button}</button>
{/if}
...
<States {state} {progress} {error}>
  ...
  <div slot="done">
    <p>{copy.done(imageCount, formatBytes(resultSize))}</p>
    {#if resultUrl}
      <a href={resultUrl} download="images.pdf">{copy.download}</a>
